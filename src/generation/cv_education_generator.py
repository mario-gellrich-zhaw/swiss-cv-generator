...
from src.generation.sampling import (_localize_credentials,
...
def _maybe_translate_degree(degree: str, language: str) -> str:
    """Translate German education degree titles to the target language when needed."""
    lang = (language or "de").lower()
    if not degree or lang == "de":
        return degree

    # Only translate if the degree looks German
    german_markers = re.search(
        r"\b(EFZ|EBA|eidg\.?|eidgen[öo]ss|beruflich|berufliche|Fachausweis|Diplom|Fähigkeitszeugnis|Berufsattest|für|mit)\b",
        degree,
        flags=re.IGNORECASE,
    )
    gendered_markers = re.search(
        r"/(?:in|frau|mann)\b", degree, flags=re.IGNORECASE)

    if german_markers or gendered_markers:
        return translate_occupation_title(degree, lang)

    return degree


def _normalize_education_language(
    education_history: List[Dict[str, Any]],
    language: str
) -> List[Dict[str, Any]]:
    """Ensure education degrees are localized for FR/IT outputs."""
    lang = (language or "de").lower()
    if lang == "de":
    ...
    for entry in education_history:
        ...
        if not degree:
            continue
        # Re-run translation for any residual German markers.
        degree = _maybe_translate_degree(degree, lang)
        degree = _localize_credentials(degree, lang)
        entry["degree"] = degree
...
def extract_education_data(occupation_doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(weiterbildung, dict):
        ...
        if isinstance(career_progression, list):
            weiterbildung_list = cast(
                List[Dict[str, Any]], education_data["weiterbildung"])
            for item in career_progression:
                if isinstance(item, dict):
                    weiterbildung_list.append({
...
def generate_education_history(
    ...
    Args:
        ...
        language: Language code (de, fr, it) for translations.
    ...
    if primary:
        ...
        if not abschluss:
            if "EFZ" in bildungstyp or "Eidgenössisches Fähigkeitszeugnis" in bildungstyp:
                efz_names = {
                    ...
                    "fr": "Certificat fédéral de capacité (CFC)",
                    "it": "Attestato federale di capacità (AFC)",
                }
                abschluss = efz_names.get(language, efz_names["de"])
        ...
        # Translate occupation-style degrees and normalize credentials by language
        abschluss = _maybe_translate_degree(abschluss, language)
        abschluss = _localize_credentials(abschluss, language)

        # Get institution name
        if not institution:
            if "FH" in bildungstyp or "Fachhochschule" in bildungstyp:
                institution_names = {
                    "de": f"Fachhochschule {canton}",
                    "fr": f"Haute école spécialisée {canton}",
                    "it": f"Scuola universitaria professionale {canton}"
                }
                institution = institution_names.get(
                    language, institution_names["de"])
            elif "ETH" in bildungstyp or "Universität" in bildungstyp:
                institution = "ETH Zürich" if canton == "ZH" else "Universität"
            else:
                # Use canton-based institution (translated)
                institution_names = {
                    "de": f"Berufsschule {canton}",
                    "fr": f"École professionnelle {canton}",
                    "it": f"Scuola professionale {canton}"
                }
                institution = institution_names.get(
                    language, institution_names["de"])
    ...
    completeness_score = occupation_doc.get("data_completeness", {}).get(
        "completeness_score", 0) if occupation_doc else 0
    ...
    if (completeness_score >= 0.8 and
        ...
        if berufsmaturitaet or random.random() < 0.5:  # 50% chance even without explicit data
            ...
            # Translate Berufsmaturität
            bm_degree_names = {
                "de": "Berufsmaturität",
                "fr": "Maturité professionnelle",
                "it": "Maturità professionale"
            }
            bm_institution_names = {
                "de": f"Berufsmaturitätsschule {canton}",
                "fr": f"École de maturité professionnelle {canton}",
                "it": f"Scuola di maturità professionale {canton}"
            }

            education_history.append({
                "degree": bm_degree_names.get(language, bm_degree_names["de"]),
                "institution": bm_institution_names.get(language, bm_institution_names["de"]),
                "location": canton,
                "start_year": bm_start,
                "end_year": bm_end,
    ...
    if weiterbildung_list and years_experience > 2:
        ...
        for i, wb in enumerate(weiterbildung_list[:num_weiterbildung]):
            if random.random() < 0.4:  # 40% chance per entry
                ...
                # Translate occupation-style titles and normalize credentials in continuing education
                wb_title = _maybe_translate_degree(wb_title, language)
                wb_title = _localize_credentials(wb_title, language)

                # Weiterbildung typically happens during career
                # Place it 1-5 years after primary education
                primary_end = education_history[0]["end_year"] if education_history else end_year
                wb_start = primary_end + \
                    random.randint(1, min(5, years_experience - 1))
    ...
    education_history = validate_education_timeline(
        education_history, persona_age, years_experience)

    # Sort by start_year
    education_history.sort(key=lambda x: x.get("start_year", 0))

    return _normalize_education_language(education_history, language)
...
def validate_education_timeline(
    ...
    for entry in education_history:
        ...
        if start_year < birth_year + min_start_age:
            ...
            end_year = start_year + \
                (end_year - entry.get("start_year", start_year))

        # Check: Education shouldn't end after current year
        if end_year > current_year:
            end_year = current_year
            start_year = end_year - \
                (entry.get("end_year", end_year) -
                 entry.get("start_year", start_year))
    ...
    for entry in education_history:
        ...
        if degree and institution:
            summary_parts.append(
                f"{degree} bei {institution} ({start_year}-{end_year})")
        elif degree:
