...
from src.generation.company_validator import (
    get_valid_company_for_occupation, remove_verschiedene_positionen_entries)
from src.generation.cv_activities_transformer import (
    extract_activities_from_occupation, filter_activities_by_career_level,
    generate_all_jobs_bullets_batch, generate_responsibilities_from_activities)
from src.generation.cv_timeline_validator import calculate_timeline_forward
...
def calculate_realistic_job_timeline(
    ...
    for job_num in range(num_jobs):
        ...
        if not is_current:
            ...
            gap_months = (current_end_year - next_start_year) * \
                12 + (current_end_month - next_start_month)
    ...
    total_periods_years = sum(int(p["duration_years"])
                              for p in periods if not bool(p["is_gap"]))
    total_periods_months = sum(int(p["duration_months"])
                               for p in periods if not bool(p["is_gap"]))
    ...
    if discrepancy > 2.0:
        ...
        if periods:
            ...
            if abs(adjustment) <= 2:
                start_year_val = first_period.get(
                    "start_year", education_end_year)
                if start_year_val is None:
                    start_year_val = education_end_year
                first_period["start_year"] = int(
                    round(int(start_year_val) - adjustment))

    return cast(List[Dict[str, Any]], periods)
...
def clean_job_text(text: str, career_level: str) -> str:
    ...
    text = re.sub(r'\b(\w+(?:\s+\w+){1,3})\s+\1\b',
...
def ensure_logical_progression(
    ...
    sorted_jobs = sorted(
        job_history,
        key=lambda j: (int(j.get("start_date", "2000-01").split("-")[0]), int(j.get(
            "start_date", "2000-01").split("-")[1]) if "-" in j.get("start_date", "") else 1)
    )
    ...
    for i, job in enumerate(real_jobs):
        ...
        if "Senior" not in base_title and "Lead" not in base_title and "Leiter" not in base_title and "Chef" not in base_title:
            if job_level == "senior" and num_real_jobs > 2:
                ...
                job["position"] = f"Leiter {base_title}" if random.random(
                ) < 0.5 else f"Lead {base_title}"
...
def generate_job_entry_fast(
    ...
    if is_current:
        ...
        if total_jobs == 2:
            ...
            job_career_level = ["junior", "mid",
                                career_level][min(job_index, 2)]
        else:
    ...
    # Get position title (keep German — translation happens after ensure_logical_progression)
    ...
    return {
        ...
        "position": position,
        "_position_job_id": position_job_id,  # Used for native title lookup after progression
        "location": canton,
...
def generate_job_entry(
    ...
    if is_gap:
        ...
        gap_label = gap_names.get(gap_type or "weiterbildung", "Weiterbildung")
        return {
            "company": gap_label,
            "position": gap_label,
    ...
    if is_current:
        ...
        if total_jobs == 2:
            ...
            if job_index == 0:
                ...
                job_career_level = "senior" if career_level in [
                    "senior", "lead"] else "mid"
            else:
                job_career_level = career_level
    ...
    # Translate position title if needed (FR/IT)
    # Use the same translate_occupation_title function to ensure consistency
    # and apply all fallback replacements
    if language and language.lower() != "de" and position:
        from src.generation.sampling import translate_occupation_title
        position = translate_occupation_title(position, language, job_id=position_job_id)
...
def get_technologies_from_skills(job_id: Optional[str], limit: int = 8) -> List[str]:
    ...
    technical_skills.sort(
        key=lambda x: next(
            (s.get("importance", 0)
             for s in skills if s.get("skill_name_de") == x),
            0
        ),
        reverse=True
    )
...
def generate_job_history(
    ...
    # Ensure types for timeline inputs
    assert isinstance(education_start_year, int)
    assert isinstance(education_duration_years, int)
    ...
    # If timeline validation failed, fall back to old method (log quietly)
    if not periods and timeline_issues:
        # Log debug but continue with fallback
        import logging
        logging.getLogger(__name__).debug(
            "Timeline validation failed: %s issues. Using fallback calculation.",
            len(timeline_issues)
        )
    ...
    for i, period in enumerate(periods):
        if period.get("is_gap", False):
            ...
            gap_label = gap_names.get(
                gap_type or "weiterbildung", "Weiterbildung")
            job_entry = {
                "company": gap_label,
                "position": gap_label,
                "location": persona.get("canton", ""),
    ...
    if real_jobs_data:
        occupation_title = persona.get(
            "occupation", occupation_doc.get("title", ""))
        all_bullets = generate_all_jobs_bullets_batch(
            real_jobs_data, occupation_title, language)
    ...
    job_history = ensure_logical_progression(
        job_history, persona.get("career_level", "mid"))

    # Translate positions to target language AFTER progression (so German prefixes are correct)
    if language and language.lower() != "de":
        from src.generation.sampling import translate_occupation_title
        for entry in job_history:
            if entry.get("category") == "gap_filler":
                entry.pop("_position_job_id", None)
                continue
            pos = entry.get("position", "")
            pos_job_id = entry.pop("_position_job_id", None)
            if pos:
                entry["position"] = translate_occupation_title(pos, language, job_id=pos_job_id)
    else:
        for entry in job_history:
            entry.pop("_position_job_id", None)
    ...
    job_history.sort(
        key=lambda x: (
            ...
            int(x.get("start_date", "2000-01").split("-")
                [1]) if "-" in x.get("start_date", "") else 1
        ),
        reverse=True
    )
    ...
    for job in job_history:
        ...
        if company and position:
            ...
            summary_parts.append(
                f"{position} bei {company} ({start_year}-{end_year})")
