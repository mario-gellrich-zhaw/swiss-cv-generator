...
from src.data.models import Language
...
# Career level prefixes added by _add_career_prefix() (queries.py) and their
# translations. "Senior" and "Lead" are universal in Swiss professional usage.
_CAREER_PREFIX_TRANSLATIONS: Dict[str, Dict[str, str]] = {
    "fr": {"Senior": "Senior", "Lead": "Lead", "Leiter/in": "Responsable", "Chef/in": "Chef/Cheffe"},
    "it": {"Senior": "Senior", "Lead": "Lead", "Leiter/in": "Responsabile", "Chef/in": "Capo"},
    "en": {"Senior": "Senior", "Lead": "Lead", "Leiter/in": "Head", "Chef/in": "Chief"},
}


def _detect_career_prefix(title: str) -> Optional[str]:
    """Detect a German career prefix prepended to an occupation title."""
    for prefix in ("Leiter/in", "Chef/in", "Senior", "Lead"):
        if title.startswith(prefix + " "):
            return prefix
    return None


def translate_occupation_title(title: Optional[str], target_language: str, job_id: Optional[str] = None) -> str:
    """
    Translate occupation title to target language.

    Uses native titles from berufsberatung.ch source data when a job_id is
    provided, falling back to OpenAI translation and regex replacements.

    Args:
        title: German occupation title (e.g., "Koch/Köchin EFZ"), or None
        target_language: Target language code ('de', 'fr', 'it', 'en')
        job_id: Optional occupation ID for direct native-title lookup.

    Returns:
        Translated occupation title, or original if translation fails or language is German
    """
    if not title or target_language.lower() == "de":
        return title or ""

    # --- Native lookup from berufsberatung.ch source ---
    if job_id:
        from src.generation.occupation_titles import (
            get_native_occupation_title, get_german_occupation_title)

        # Detect career prefix that may have been prepended to the German title
        prefix_de = _detect_career_prefix(title)

        # Determine which job_id to look up (prefix may have been on base title)
        native = get_native_occupation_title(job_id, target_language)
        if native:
            if prefix_de:
                lang_prefix = _CAREER_PREFIX_TRANSLATIONS.get(
                    target_language.lower(), {}).get(prefix_de, prefix_de)
                return _localize_credentials(f"{lang_prefix} {native}", target_language)
            return _localize_credentials(native, target_language)

    result = title

    # Try OpenAI translation first, but it may be incomplete
    try:
        if is_openai_available() and get_openai_client():
            client = get_openai_client()
            resp = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system",
                        "content": "You are a Swiss occupation title translator. Translate ALL German occupation words and terms into the target language. This includes titles like 'Leiter/in', 'Bäcker', 'Koch', 'Informatiker' and phrases like 'eidg. Fachausweis'. Replace EFZ with CFC (French) or AFC (Italian). Return ONLY the translated title, no quotes or explanations."},
                    {"role": "user", "content": f"Translate this into {target_language.upper()}: {title}"}
                ],
                temperature=0.2,
                max_tokens=80,
            )
        ...
        # Log translation failure but continue with fallback
        import logging
        logging.debug(
            f"Translation failed for '{title}' to {target_language}: {e}")

    # ALWAYS apply fallback replacements to ensure all German words are translated
    # This handles cases where OpenAI translation was incomplete or failed
    replacements = {
        "fr": {
            "Bereichsleiter": "Responsable de domaine",
            "Leiter": "Responsable",
            ...
            "mit": "avec",
            "eidg": "fédéral",
            "Fachausweis": "Brevet professionnel",
            "Diplom": "Diplôme",
        },
        "it": {
            "Bereichsleiter": "Responsabile di area",
            "Leiter": "Responsabile",
            ...
            "Bäcker": "Fornaio",
            "Bäuerin": "Contadina",
            "Haushaltleiter": "Capo famiglia",
            "Koch": "Cuoco",
            "Köchin": "Cuoca",
            "Informatiker": "Informatico",
            "Informatikerin": "Informatica",
            "Krankenpfleger": "Infermiere",
            "Krankenpflegerin": "Infermiera",
            "Elektrotechniker": "Elettricista",
            ...
            "mit": "con",
            "eidg": "federale",
            "Fachausweis": "Attestato professionale",
            "Diplom": "Diploma",
        }
    }

    for key, value in replacements.get(target_language.lower(), {}).items():
        # Case-insensitive replacement - use flexible boundary pattern
        # This handles words with slashes like "Bereichsleiter/in"
        pattern = r'\b' + re.escape(key) + r'(?:[/\-]?i[n]?)?\b'
        result = re.sub(pattern, value, result, flags=re.IGNORECASE)

    # Always ensure credentials are localized
    return _localize_credentials(result, target_language)


def _localize_credentials(text: str, target_language: str) -> str:
    """Replace German credential abbreviations with official equivalents for the target language.

    Official Swiss mappings:
      EFZ → CFC (FR), AFC (IT)
      EBA → AFP (FR), CFP (IT)
      BP  → BF  (FR), APF (IT)
      HFP → DF  (FR), EPS (IT)
    ...
    _CREDENTIAL_MAP = {
        "fr": [("EFZ", "CFC"), ("EBA", "AFP"), ("BP", "BF"), ("HFP", "DF"), ("HF", "ES")],
        "it": [("EFZ", "AFC"), ("EBA", "CFP"), ("BP", "APF"), ("HFP", "EPS"), ("HF", "SSS")],
    }

    for de_abbr, target_abbr in _CREDENTIAL_MAP.get(lang, []):
        text = re.sub(r'\b' + de_abbr + r'\b', target_abbr, text)
...
class SamplingEngine:
    ...
    def sample_persona(self, preferred_canton=None, preferred_industry=None) -> Dict[str, Any]:
        ...
        occupation_title = str(occupation_doc.get(
            "title")) if occupation_doc and occupation_doc.get("title") else f"{career_level.capitalize()} Worker"
        ...
        # Step 9b: Translate occupation title to target language
        occupation_title = translate_occupation_title(
            occupation_title, language_str, job_id=job_id)
        ...
        # Fallback skills if database has no entries for this occupation
        if not skills_list:
            generic_skills = [
                "Teamfähigkeit",
                "Kommunikation",
            ...
            skills_list = random.sample(generic_skills, k=6)
    ...
    def sample_batch_with_demographics(self, count: int) -> List[Dict[str, Any]]:
        ...
        # Normalize industry keys to strings to avoid None entries
        industries = Counter((p.get("industry") or "other") for p in personas)
        ...
        for ind, count_ind in industries.most_common(5):
            ...
            ind_str = ind if isinstance(ind, str) else str(ind)
            emp_pct = get_industry_employment_percentage(ind_str)
            print(
