    ...
    try:
        ...
        try:
            ...
            if settings.openai_api_key:
                ...
                LOGGER.debug(
                    "Initialized modern OpenAI client (openai >= 1.0.0)")
        except ImportError:
            ...
            try:
                ...
                if settings.openai_api_key:
                    ...
                    _openai_available = True
                    LOGGER.debug(
                        "Initialized legacy OpenAI client (openai 0.28.x)")
            except ImportError:
...
def call_openai_chat(
    ...
    if _openai_client and hasattr(_openai_client, 'chat'):
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                LOGGER.warning(
                    "OpenAI modern client attempt %d failed: %s", attempt, e)
    ...
    try:
        ...
        chat_completion = getattr(openai_module, "ChatCompletion", None)
        if chat_completion is None or not hasattr(chat_completion, "create"):
            raise RuntimeError("Legacy ChatCompletion API is not available")
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                LOGGER.warning(
                    "OpenAI legacy client attempt %d failed: %s", attempt, e)
                if attempt == MAX_RETRIES or not _is_transient_error(e):
                    raise
                _sleep_with_backoff(attempt)
    except ImportError:
    ...
    raise RuntimeError(
        "OpenAI call failed: no working client available or all retries exhausted")
