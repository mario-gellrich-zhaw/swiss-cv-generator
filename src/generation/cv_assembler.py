...
from src.database.queries import (get_canton_by_code, get_occupation_by_id,
...
from src.generation.cv_continuing_education import \
    generate_additional_education
...
from src.generation.openai_client import get_openai_client, is_openai_available
...
# Prevent hanging OpenAI requests during batch generation.
...
def validate_persona_before_assembly(
        ...
        if len(path_parts) >= 2:
            ...
            if path_age_group != age_group:
                ...
                if new_portrait:
                    ...
                    issues.append(
                        f"Portrait age mismatch: resampled from {age_group}")
                else:
                    issues.append(
                        f"Warning: No portrait available for {gender}/{age_group}")
    ...
    if discrepancy > 3:
        ...
        if actual_total < expected_total:
            # Increase experience
            fixed_persona["years_experience"] = min(
                years_experience + discrepancy - 1, age - 18)
        elif actual_total > expected_total:
            # Decrease experience
            fixed_persona["years_experience"] = max(
                years_experience - discrepancy + 1, 0)
...
def generate_personalized_languages(
    ...
    # Display language names and proficiency labels in the CV's language.
    # Note: primary_language here is used as the CV language by the caller.
    if primary_language == "fr":
        lang_names = {"de": "Allemand", "fr": "Français", "it": "Italien",
                      "en": "Anglais", "es": "Espagnol", "pt": "Portugais"}
        prof = {"native": "Langue maternelle",
                "fluent": "Courant", "good": "Bon", "basic": "Notions"}
    elif primary_language == "it":
        lang_names = {"de": "Tedesco", "fr": "Francese", "it": "Italiano",
                      "en": "Inglese", "es": "Spagnolo", "pt": "Portoghese"}
        prof = {"native": "Madrelingua", "fluent": "Fluente",
                "good": "Buono", "basic": "Conoscenze di base"}
    else:
        lang_names = {"de": "Deutsch", "fr": "Französisch", "it": "Italienisch",
                      "en": "Englisch", "es": "Spanisch", "pt": "Portugiesisch"}
        prof = {"native": "Muttersprache", "fluent": "Fließend",
                "good": "Gut", "basic": "Grundkenntnisse"}
    ...
    if canton_doc:
        ...
        if primary_pct > 70:
            languages.append(
                f"{lang_names.get(primary_language, primary_language)} ({prof['native']})")
        else:
            languages.append(
                f"{lang_names.get(primary_language, primary_language)} ({prof['fluent']})")

        # Add secondary Swiss languages
        if primary_language == "de":
            # Deutschschweiz: +Französisch
            if lang_fr > 10:
                proficiency = random.choice([prof["good"], prof["basic"]])
                languages.append(
                    f"{lang_names.get('fr', 'Französisch')} ({proficiency})")
        elif primary_language == "fr":
            # Romandie: +Deutsch
            if lang_de > 10:
                proficiency = random.choice([prof["good"], prof["fluent"]])
                languages.append(
                    f"{lang_names.get('de', 'Deutsch')} ({proficiency})")
        elif primary_language == "it":
    ...
    if age_group == "18-25":
        if random.random() < 0.8:
            languages.append(
                f"{lang_names.get('en', 'Englisch')} ({prof['fluent']})")
        else:
            languages.append(
                f"{lang_names.get('en', 'Englisch')} ({prof['good']})")
    elif age_group == "26-40":
        if random.random() < 0.6:
            proficiency = random.choice([prof["fluent"], prof["good"]])
            languages.append(
                f"{lang_names.get('en', 'Englisch')} ({proficiency})")
        else:
            languages.append(
                f"{lang_names.get('en', 'Englisch')} ({prof['basic']})")
    else:  # 41-65
        if random.random() < 0.4:
            proficiency = random.choice([prof["good"], prof["basic"]])
            languages.append(
    ...
    if random.random() < 0.2:
        fourth_code = random.choice(["es", "it", "pt"])
        fourth_lang = lang_names.get(fourth_code, fourth_code)
        proficiency = random.choice([prof["basic"], prof["good"]])
...
def generate_personalized_hobbies(
    ...
    # Regional base hobbies (translated)
    regional_hobbies = {
        "de": ["Wandern", "Skifahren", "Jodeln", "Alphornblasen", "Schwingen"],
        "fr": ["Randonnée", "Ski", "Dégustation de vins", "Voile"],
        "it": ["Escursionismo", "Sci", "Visite ai grotti", "Mountain bike"]
    }
    ...
    # Age-specific hobbies (translated)
    age_hobbies = {
        "de": {
        ...
        "fr": {
            "18-25": ["Jeux vidéo", "Fitness", "Voyages", "Festivals", "Escalade"],
            "26-40": ["Yoga", "Cuisine", "Photographie", "Craft Beer", "Cyclisme", "Course à pied"],
            "41-65": ["Golf", "Jardinage", "Musique classique", "Échecs", "Randonnée", "Lecture"]
        },
        "it": {
            "18-25": ["Videogiochi", "Fitness", "Viaggi", "Festival", "Arrampicata"],
            "26-40": ["Yoga", "Cucina", "Fotografia", "Craft Beer", "Ciclismo", "Corsa"],
            "41-65": ["Golf", "Giardinaggio", "Musica classica", "Scacchi", "Escursionismo", "Lettura"]
        }
    }

    lang_age_hobbies = age_hobbies.get(language, age_hobbies["de"])
    age_list = lang_age_hobbies.get(age_group, lang_age_hobbies["26-40"])
    hobbies.extend(random.sample(age_list, min(2, len(age_list))))

    # Occupation-specific hobbies (translated)
    occupation_hobbies = {
        "de": {
        ...
        "fr": {
            "technical": ["Impression 3D", "Électronique", "Open Source", "Programmation"],
            "creative": ["Peinture", "Photographie", "Musique", "Design"],
            "social": ["Travail associatif", "Bénévolat", "Mentorat"]
        },
        "it": {
            "technical": ["Stampa 3D", "Elettronica", "Open Source", "Programmazione"],
            "creative": ["Pittura", "Fotografia", "Musica", "Design"],
            "social": ["Lavoro associativo", "Volontariato", "Mentoring"]
        }
    }

    lang_occ_hobbies = occupation_hobbies.get(
        language, occupation_hobbies["de"])
    if occupation_type in lang_occ_hobbies:
        occ_list = lang_occ_hobbies[occupation_type]
    ...
    if len(unique_hobbies) < 4:
        ...
        unique_hobbies.extend(random.sample(
            remaining, min(4 - len(unique_hobbies), len(remaining))))
...
def generate_varied_summary(
    ...
    name_key = {"de": "skill_name_de", "fr": "skill_name_fr",
                "it": "skill_name_it"}.get(language, "skill_name_de")
    actual_skills = [s.get(name_key) or s.get("skill_name_de", "")
                     for s in skills_docs[:3] if (s.get(name_key) or s.get("skill_name_de"))]
...
def score_cv_quality(cv_doc: CVDocument) -> Dict[str, Any]:
    ...
    if cv_doc.jobs:
        current_job = next(
            (j for j in cv_doc.jobs if j.get("is_current")), None)
        if current_job:
            responsibilities = current_job.get("responsibilities", [])
            if len(responsibilities) < 3:
                completeness_score -= 10
                completeness_issues.append(
                    "Insufficient responsibilities in current job")
    ...
    impact_keywords = ["reduzierte", "steigerte",
                       "optimierte", "verbesserte", "erhöhte"]
...
def categorize_skills(skills: List[Dict[str, Any]], language: str = "de") -> Dict[str, List[str]]:
    """
    Categorize skills into technical, soft, and languages.

    Args:
        skills: List of skill dictionaries from occupation_skills.
        language: Target language (de, fr, it).

    Returns:
        Dictionary with categorized skills.
        Returns empty dict if DB doesn't have translations for requested language
        (to trigger fallback lists in generate_complete_cv).
    """
    ...
    name_key = {
        ...
        "fr": "skill_name_fr",
        "it": "skill_name_it",
    }.get(language, "skill_name_de")

    # Check if DB has translations for requested language
    # If less than 50% translated, return empty dict to trigger fallback lists
    if language != "de" and skills:
        translated_count = sum(1 for skill in skills if skill.get(name_key))
        translation_ratio = translated_count / len(skills) if skills else 0

        if translation_ratio < 0.5:
            # Not enough translations, use fallback lists
            return categorized

    for skill in skills:
        category = skill.get("skill_category", "soft")
        # For DE, use skill_name_de. For FR/IT, use localized field
        if language == "de":
            skill_name = skill.get("skill_name_de", "")
        else:
            skill_name = skill.get(name_key, "")
...
def get_languages_for_cv(canton: str, primary_language: str) -> List[str]:
    ...
    languages.append(
        f"{lang_names.get(primary_language, primary_language)} (Muttersprache)")
        ...
        for lang in other_langs:
            if lang != primary_language:
                proficiency = random.choice(
                    ["Fließend", "Gut", "Grundkenntnisse"])
                languages.append(
                    f"{lang_names.get(lang, lang)} ({proficiency})")
...
def generate_complete_cv(persona: Dict[str, Any]) -> Tuple[Optional[CVDocument], Optional[Dict[str, Any]]]:
    ...
    portrait_path = persona.get("portrait_path")
    portrait_base64 = load_portrait_image(
        portrait_path, resize=(150, 150), circular=True)

    # 3. Summary (varied, specific)
    summary = generate_varied_summary(persona, occupation_doc, language)

    # 4. Education history
    education_history = generate_education_history(
        persona, occupation_doc, language)
    ...
    for job in job_history:
        ...
        if job.get("is_current", False):
            ...
            previous_level = "mid" if persona.get("career_level") in [
                "senior", "lead"] else "junior"
            responsibilities = generate_responsibilities_from_activities(
                job_id,
                previous_level,
                job.get("company", ""),
    ...
    if isinstance(skills_list, list) and skills_list and isinstance(skills_list[0], str):
        # Skills are strings; prefer DB categories, fall back to soft skills list.
        from src.database.queries import get_skills_by_occupation
        skills_docs = get_skills_by_occupation(job_id) if job_id else []
        categorized_skills = categorize_skills(skills_docs, language)

        # If the occupation has no categorized skills, map provided strings to soft skills.
        if not categorized_skills.get("technical") and not categorized_skills.get("soft"):
            # If we only have raw strings (unknown language), prefer localized defaults
            localized_fallbacks = {
                "de": ["Teamfähigkeit", "Kommunikation", "Selbstorganisation", "Problemlösung", "Projektmanagement", "MS Office", "Zeitmanagement", "Kundenorientierung"],
                "fr": ["Travail d'équipe", "Communication", "Autonomie", "Résolution de problèmes", "Gestion de projet", "MS Office", "Gestion du temps", "Orientation client"],
                "it": ["Lavoro di squadra", "Comunicazione", "Autonomia", "Problem solving", "Gestione progetti", "MS Office", "Gestione del tempo", "Orientamento al cliente"],
            }
            fallbacks = localized_fallbacks.get(
                language, localized_fallbacks["de"])
            categorized_skills = {
                "technical": [],
                "soft": fallbacks[:8],
                "languages": [],
        ...
        # Skills are dictionaries or empty list - try DB first
        from src.database.queries import get_skills_by_occupation

        # First, try to categorize what we have in skills_list
        categorized_skills = categorize_skills(skills_list, language) if skills_list else {
            "technical": [],
            "soft": [],
            "languages": []
        }

        # If we still don't have skills, try to get them from DB
        if (not categorized_skills.get("technical") and
                not categorized_skills.get("soft") and job_id):
            skills_docs = get_skills_by_occupation(job_id)
            categorized_skills = categorize_skills(skills_docs, language)

    # Add languages to skills (personalized)
    languages = generate_personalized_languages(
        canton, language, persona.get("age", 25))
    categorized_skills["languages"] = languages

    # Absolute fallback to guarantee Kompetenzen (localized)
    if not categorized_skills.get("technical") and not categorized_skills.get("soft"):
        localized_fallbacks = {
            "de": ["Teamfähigkeit", "Kommunikation", "Selbstorganisation", "Problemlösung", "Projektmanagement", "MS Office", "Zeitmanagement", "Kundenorientierung", "Organisation", "Verlässlichkeit"],
            "fr": ["Travail d'équipe", "Communication", "Autonomie", "Résolution de problèmes", "Gestion de projet", "MS Office", "Gestion du temps", "Orientation client", "Organisation", "Fiabilité"],
            "it": ["Lavoro di squadra", "Comunicazione", "Autonomia", "Problem solving", "Gestione progetti", "MS Office", "Gestione del tempo", "Orientamento al cliente", "Organizzazione", "Affidabilità"],
        }
        fallback_skills = localized_fallbacks.get(
            language, localized_fallbacks["de"])
        categorized_skills["technical"] = []
        categorized_skills["soft"] = random.sample(
            fallback_skills, k=min(6, len(fallback_skills)))
    ...
    hobbies = generate_personalized_hobbies(
        canton, language, age_group, occupation_type)
    ...
    cv_doc = CVDocument(
        ...
        current_title=persona.get(
            "current_title", persona.get("occupation", "")),
        industry=persona.get("industry", ""),
        career_level=persona.get("career_level", "mid"),
        years_experience=persona.get("years_experience", 0),
        summary=summary,
