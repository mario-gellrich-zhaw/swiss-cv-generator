...
from src.generation.cv_assembler import CVDocument, generate_complete_cv
...
def export_cv_pdf(cv_doc: CVDocument, output_path: Path, template_name: Optional[str] = "classic") -> Path:
    ...
    """
    try:
        from src.export.pdf_templates import (get_available_templates,
                                              render_cv_with_template)
    ...
    try:
        ...
        from reportlab.platypus import Image  # type: ignore[import]
        ...
        if cv_doc.portrait_base64:
            try:
                # Decode base64 image
                img_data = base64.b64decode(cv_doc.portrait_base64.split(
                    ',')[1] if ',' in cv_doc.portrait_base64 else cv_doc.portrait_base64)
        ...
        if cv_doc.jobs:
            ...
            today_label = {
                ...
                "fr": "Aujourd'hui",
                "it": "Oggi",
            }.get(cv_doc.language, "Heute")
            tech_label = {
                "de": "Technologien",
                "fr": "Technologies",
                "it": "Tecnologie",
            }.get(cv_doc.language, "Technologien")
            ...
            for job in cv_doc.jobs:
                ...
                end_date = job.get('end_date', '') if not job.get(
                    'is_current', False) else today_label
                ...
                if technologies:
                    ...
                    elems.append(
                        Paragraph(f"<i>{tech_label}: {tech_str}</i>", normal))
        ...
        if cv_doc.education:
            ...
            for edu in cv_doc.education:
                ...
                year_range = f"{start_year} – {end_year}" if start_year and end_year else str(
                    end_year) if end_year else ""
        ...
        if cv_doc.skills:
            ...
            if technical:
                ...
                # Limit to 15
                elems.append(Paragraph(", ".join(technical[:15]), normal))
                elems.append(Spacer(1, 4))
            ...
            if soft:
                ...
                # Limit to 10
                elems.append(Paragraph(", ".join(soft[:10]), normal))
                elems.append(Spacer(1, 4))
...
def export_cv_docx(cv_doc: CVDocument, output_path: Path) -> Path:
    ...
    try:
        ...
        }.get(cv_doc.language, "Persönliche Angaben")
        ...
        def lp(key: str) -> str:
            return labels_personal.get(key, {}).get(cv_doc.language, labels_personal.get(key, {}).get("de", key))

        doc.add_heading(heading_personal, 1)
        p = doc.add_paragraph()
        p.add_run(f"{lp('name')}: {cv_doc.full_name}\n")
        p.add_run(f"{lp('age')}: {cv_doc.age}\n")
        p.add_run(f"{lp('location')}: {cv_doc.city}, {cv_doc.canton}\n")
        if cv_doc.email:
            p.add_run(f"{lp('email')}: {cv_doc.email}\n")
        if cv_doc.phone:
            p.add_run(f"{lp('phone')}: {cv_doc.phone}\n")

        # Summary
        if cv_doc.summary:
            doc.add_heading("Zusammenfassung" if cv_doc.language ==
                            "de" else "Résumé" if cv_doc.language == "fr" else "Riassunto", 1)
            doc.add_paragraph(cv_doc.summary)

        # Professional Experience
        if cv_doc.jobs:
            doc.add_heading("Berufserfahrung" if cv_doc.language ==
                            "de" else "Expérience professionnelle" if cv_doc.language == "fr" else "Esperienza professionale", 1)
            for job in cv_doc.jobs:
                p = doc.add_paragraph()
                p.add_run(
                    f"{job.get('position', '')} - {job.get('company', '')}").bold = True
                today_label = {
                    "de": "Heute",
                    "fr": "Aujourd'hui",
                    "it": "Oggi",
                ...
                end_date = job.get('end_date', today_label)
                if job.get('is_current', False):
                    end_date = today_label

                p.add_run(
                    f"\n{job.get('start_date', '')} - {end_date}\n")
        ...
        # Education
        if cv_doc.education:
            doc.add_heading("Ausbildung" if cv_doc.language ==
                            "de" else "Formation" if cv_doc.language == "fr" else "Formazione", 1)
            for edu in cv_doc.education:
                p = doc.add_paragraph()
                p.add_run(
        ...
        # Skills
        if cv_doc.skills:
            doc.add_heading("Kompetenzen" if cv_doc.language ==
                            "de" else "Compétences" if cv_doc.language == "fr" else "Competenze", 1)
            ...
            def cat_label(cat: str) -> str:
                return category_labels.get(cat, {}).get(cv_doc.language, cat.capitalize())

            for category, skills_list in cv_doc.skills.items():
                if skills_list:
                    doc.add_heading(cat_label(category), 2)
        ...
        console.print(
            "[yellow]python-docx not installed. Saving as JSON instead.[/yellow]")
...
def generate(
    ...
    console.print(Panel.fit(
        "[bold green]🇨🇭 Swiss CV Generator[/bold green]", border_style="green"))
    ...
    with Progress(
        ...
        while generated < count and attempts < max_attempts:
            ...
            try:
                ...
                # Force requested language early so downstream generation (summary, bullets, etc.)
                # happens in the correct locale instead of being generated in DE and only relabelled later.
                if language:
                    persona["language"] = language

                    # Re-sample names to match requested language
                    from src.database.queries import (sample_first_name,
                                                      sample_last_name)
                    from src.generation.sampling import \
                        translate_occupation_title
                    gender = persona.get("gender", "male")

                    try:
                        first_name_doc = sample_first_name(language, gender)
                        if first_name_doc:
                            persona["first_name"] = first_name_doc.get(
                                "name", persona.get("first_name", "Unknown"))
                    except Exception:
                        pass  # Keep original name if re-sampling fails

                    try:
                        last_name_doc = sample_last_name(language)
                        if last_name_doc:
                            persona["last_name"] = last_name_doc.get(
                                "name", persona.get("last_name", "Unknown"))
                            # Update full_name as well
                            persona["full_name"] = f"{persona.get('first_name', '')} {persona.get('last_name', '')}"
                    except Exception:
                        pass  # Keep original name if re-sampling fails

                    # Re-translate occupation title to match requested language
                    # (original was translated based on canton, not requested language)
                    try:
                        job_id = persona.get("job_id")
                        if job_id:
                            occ_doc = get_occupation_by_id(job_id)
                            if occ_doc:
                                # Get original German title
                                original_title = occ_doc.get("title", "")
                                if original_title:
                                    # Translate to requested language
                                    translated_title = translate_occupation_title(
                                        original_title, language, job_id=job_id)
                                    persona["occupation"] = translated_title
                                    # IMPORTANT: Also update current_title which is used in PDF header!
                                    persona["current_title"] = translated_title
                    except Exception:
                        pass  # Keep original occupation if re-translation fails
                ...
                progress.update(
                    task, description=f"[cyan]Generating: {current_name}...")

                # Load occupation document
                job_id = persona.get("job_id")
                occupation_doc = get_occupation_by_id(
                ...
                if cv_doc is None:
                    ...
                    if verbose:
                        issues = quality_report.get('issues', [])[
                            :3] if quality_report else []
                        console.print(
                            f"[yellow]CV generation failed quality check for {current_name}: {issues}[/yellow]")
                    continue

                # Get quality score from report
                quality_score = quality_report.get("scores", {}).get(
                    "overall", 100.0) if quality_report else 100.0
                ...
                if validate_timeline:
                    try:
                        ...
                        if verbose and issues:
                            console.print(
                                f"[yellow]Timeline issues for {current_name}: {len(issues)}[/yellow]")

                    except Exception as e:
                        if strict:
                            console.print(
                                f"[red]Timeline validation failed for {current_name}: {e}[/red]")
                            continue
                        else:
                            if verbose:
                                console.print(
                                    f"[yellow]Timeline validation warning for {current_name}: {e}[/yellow]")
                ...
                if not quality_passed:
                    ...
                    if verbose:
                        console.print(
                            f"[yellow]Quality score below threshold for {current_name}: {quality_score:.1f} < {min_quality_score}[/yellow]")
                    continue
                ...
                if format in ('pdf', 'both'):
                    ...
                    if verbose:
                        console.print(
                            f"[green]✓ PDF ({chosen_template}): {pdf_path}[/green]")
                ...
                stats["by_industry"][cv_doc.industry] = stats["by_industry"].get(
                    cv_doc.industry, 0) + 1
                stats["by_career_level"][cv_doc.career_level] = stats["by_career_level"].get(
                    cv_doc.career_level, 0) + 1
                age_grp = get_age_group(cv_doc.age)
                stats["by_age_group"][age_grp] = stats["by_age_group"].get(
                    age_grp, 0) + 1
    ...
    if stats["by_industry"]:
        industry_table = Table(title="By Industry",
    ...
    if stats["by_age_group"]:
        age_table = Table(title="By Age Group",
                          show_header=True, header_style="bold yellow")
