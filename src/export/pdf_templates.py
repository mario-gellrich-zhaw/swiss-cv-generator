...
from reportlab.pdfbase import pdfmetrics, ttfonts  # type: ignore
...
def _get(p: Any, key: str, default: Any = None) -> Any:
    """Safely get attribute or dict value, preserving original type."""
    if p is None:
        return default
...
def _format_end_date(end_date: Any, lang: str = "de") -> str:
    """Format end date, localized for current jobs."""
    if end_date is None or end_date == "" or str(end_date).lower() == "none":
        ...
        return mapping.get((lang or "de")[:2], "Heute")
    return str(end_date)


def _skills_placeholder(lang: str) -> str:
    mapping = {
        "de": "Skills verfügbar",
        "fr": "Compétences disponibles",
        "it": "Competenze disponibili",
    }
    return mapping.get((lang or "de")[:2], "Skills verfügbar")
...
def render_classic(cv_doc: Any, out_path: str):
    ...
    # Header
    name = _get(
        cv_doc, "full_name") or f"{_get(cv_doc, 'first_name')} {_get(cv_doc, 'last_name')}"
    ...
    if summary:
        left_content.append(
            AccentBar(LEFT_COL_WIDTH * 0.2, 2, HexColor(accent)))
    ...
    # Experience
    jobs: List[Dict[str, Any]] = _get(cv_doc, "jobs") or []
    real_jobs = [j for j in jobs if not (isinstance(
        j, dict) and j.get("category") == "gap_filler")]
    ...
    if real_jobs:
        ...
        for job in real_jobs:
            position = job.get("position", "") if isinstance(
                job, dict) else getattr(job, "position", "")
            company = job.get("company", "") if isinstance(
                job, dict) else getattr(job, "company", "")
            start = job.get("start_date", "") if isinstance(
                job, dict) else getattr(job, "start_date", "")
            end_raw = job.get("end_date") if isinstance(
                job, dict) else getattr(job, "end_date", None)
            end = _format_end_date(end_raw, language)
            resps = job.get("responsibilities", []) if isinstance(
                job, dict) else getattr(job, "responsibilities", [])

            left_content.append(Paragraph(position, styles["job_title"]))
            left_content.append(
                Paragraph(f"{company}  |  {start} – {end}", styles["company"]))
    ...
    # Education
    edu: List[Dict[str, Any]] = _get(cv_doc, "education") or []
    if edu:
        left_content.append(
            AccentBar(LEFT_COL_WIDTH * 0.2, 2, HexColor(accent)))
        left_content.append(Spacer(1, 4))
        ...
        for e in edu:
            degree = e.get("degree", "") if isinstance(
                e, dict) else getattr(e, "degree", "")
            inst = e.get("institution", "") if isinstance(
                e, dict) else getattr(e, "institution", "")
            sy = e.get("start_year", "") if isinstance(
                e, dict) else getattr(e, "start_year", "")
            ey = e.get("end_year", "") if isinstance(
                e, dict) else getattr(e, "end_year", "")

            left_content.append(Paragraph(degree, styles["job_title"]))
            left_content.append(
                Paragraph(f"{inst}  |  {sy} – {ey}", styles["company"]))
            left_content.append(Spacer(1, 4))
    ...
    if temp_portrait:
        right_content.append(
            Image(temp_portrait, width=RIGHT_COL_WIDTH - 4, height=RIGHT_COL_WIDTH - 4))
    ...
    # Skills - Always rendered (guaranteed non-empty from cv_assembler)
    # Try to get skills dict directly from object
    skills_raw = _get(cv_doc, "skills")
    skills: Any = {}
    if isinstance(skills_raw, dict):
        skills = skills_raw
    elif hasattr(cv_doc, 'skills'):
        skills = cv_doc.skills if isinstance(cv_doc.skills, dict) else {}
    ...
    if all_skills:
        for s in all_skills[:6]:
            right_content.append(
                Paragraph(f"• {s}", styles["bullet_small"]))
    else:
        right_content.append(
            Paragraph(f"• {_skills_placeholder(language)}", styles["bullet_small"]))
    right_content.append(Spacer(1, 8))
    ...
    if isinstance(skills, dict) and "languages" in skills:
        ...
        for lang in skills["languages"][:3]:
            right_content.append(
                Paragraph(f"• {lang}", styles["bullet_small"]))
        right_content.append(Spacer(1, 8))

    # Hobbies
    hobbies: List[str] = _get(cv_doc, "hobbies") or []
    if hobbies:
        right_content.append(Paragraph(labels["hobbies"], styles["h_right"]))
        right_content.append(
            Paragraph(", ".join(hobbies[:4]), styles["small"]))
...
def render_modern(cv_doc: Any, out_path: str):
    ...
    sidebar_frame = Frame(MARGIN, MARGIN, SIDEBAR_WIDTH -
                          2*MARGIN, PAGE_HEIGHT - 2*MARGIN, id="sidebar")
    content_frame = Frame(SIDEBAR_WIDTH + MARGIN, MARGIN, CONTENT_WIDTH - 2*MARGIN,
                          PAGE_HEIGHT - 2*MARGIN, id="content")

    doc.addPageTemplates([PageTemplate(id="Modern", frames=[
                         sidebar_frame, content_frame], onPage=draw_sidebar)])
    ...
    temp_portrait = _save_portrait_temp(_get(cv_doc, "portrait_base64"))
    if temp_portrait:
        flow.append(Image(temp_portrait, width=SIDEBAR_WIDTH -
                    3*MARGIN, height=SIDEBAR_WIDTH - 3*MARGIN))
    ...
    if isinstance(skills, dict):
        for cat, lst in skills.items():
            if isinstance(lst, list) and cat != "languages":
                all_skills.extend([str(s) for s in lst])
    ...
    if all_skills:
        ...
        flow.append(
            Paragraph(f"• {_skills_placeholder(language)}", styles["bullet_w"]))
...
def render_minimal(cv_doc: Any, out_path: str):
    ...
    if temp_portrait:
        ...
        city = _get(cv_doc, "city")
        canton = _get(cv_doc, "canton")
        contact_parts = [x for x in [email, phone,
                                     f"{city}, {canton}" if city else canton] if x]
        title_content.append(
            Paragraph(" • ".join(contact_parts), styles["contact"]))

        header_data = [[title_content, Image(
            temp_portrait, width=32*mm, height=32*mm)]]
    ...
    if jobs:
        ...
        for job in jobs:
            ...
            flow.append(
                Paragraph(f"{position} — {company}", styles["job_header"]))
    ...
    if True:  # Always render
        ...
        if all_skills:
            ...
            left = [_skills_placeholder(language)]
            right = []
        skill_data = []
        for i in range(max(len(left), len(right))):
            l = left[i] if i < len(left) else ""
            r = right[i] if i < len(right) else ""
            skill_data.append([Paragraph(l, styles["normal"]),
                              Paragraph(r, styles["normal"])])
        skill_table = Table(skill_data, colWidths=[
...
def render_timeline(cv_doc: Any, out_path: str):
    ...
    if temp_portrait:
        ...
        phone = _get(cv_doc, "phone")
        title_content.append(
            Paragraph(f"{email} • {phone}", styles["contact"]))

        header_data = [
            [Image(temp_portrait, width=28*mm, height=28*mm), title_content]]
    ...
    if jobs:
        ...
        for job in jobs:
            ...
            timeline_row = Table(timeline_data, colWidths=[
                                 DATE_COL, CONTENT_WIDTH - DATE_COL])
    ...
    if all_skills:
        ...
        flow.append(Paragraph("• Skills verfügbar", styles["normal"]))
