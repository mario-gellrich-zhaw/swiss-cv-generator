...
A comprehensive system for generating realistic, demographically authentic Swiss CVs.
...
3. **Automatic Setup:**
   ...
   - ✅ **Restores complete database from snapshots** — cantons, names, companies, skills (~30 seconds, no OpenAI needed)
   - ✅ Tests database connection

   **No OpenAI API key is required** for initial setup. All collections are pre-built and stored as JSON snapshots in the repository (`data/swiss_cv_generator/`).

4. **Optional: Add OpenAI API Key**
   An API key is only needed if you want to **regenerate** names/companies/cantons from scratch, or for AI-enhanced CV summaries:
   ```bash
...
**Note:** The Codespaces setup is fully automated and requires no OpenAI API key. After the container builds, everything is ready to generate CVs. The setup includes:
- MongoDB installation and startup
- Python dependencies installation
- CV_DATA database import (~1,851 occupations)
- Instant database restore from pre-built snapshots (cantons, names, companies, skills)
...
# 3. Import source occupations (CV_DATA)
python scripts/import_cv_data.py

# 4. Import pre-built database snapshots (fast, no OpenAI needed)
...
**Important:** Both databases must be populated before CV generation can work. Use the included JSON snapshots (fast, no API key needed) or regenerate from scratch with the setup scripts.
...
│   ├── source/                   # Original source files (incl. multilingual occupation titles)
│   ├── processed/                # Processed datasets
│   ├── swiss_cv_generator/       # Pre-built DB snapshots (committed to git)
...
**Note:** Both databases are included as JSON snapshots and imported automatically during Codespaces setup — no scraper and no OpenAI API key required!
- `data/CV_DATA.cv_berufsberatung.json` — source occupations (~1,851 entries)
- `data/swiss_cv_generator/*.json` — pre-built collections (cantons, names, companies, skills)
...
### 1. Import Source Occupations (CV_DATA)

The source occupation database is included as a JSON file — no scraper needed:

```bash
python scripts/import_cv_data.py
```

Imports ~1,851 occupations from `data/CV_DATA.cv_berufsberatung.json` in a few seconds.

**Alternative:** Scrape fresh data from berufsberatung.ch (takes 25–45 minutes):
```bash
...
**Option A: Restore from snapshots (recommended — fast, no OpenAI needed)**

Pre-built collections are included in the repository (`data/swiss_cv_generator/`):

```bash
python scripts/import_database.py
```

Restores all 6 collections (cantons, names, companies, skills, etc.) in seconds. Existing collections are skipped automatically; use `--force` to reimport.

**Option B: Full setup from scratch (requires OpenAI API key)**

If you want to regenerate the data (e.g. to expand names or companies):

```bash
python scripts/setup_complete_database.py
```

Orchestrates AI-powered generation of names, companies, cantons and skill extraction. Takes 5–15 minutes.

After regenerating, update the snapshots so new Codespaces benefit too:

```bash
python scripts/export_database.py
git add data/swiss_cv_generator/
git commit -m "Update DB snapshots"
```
...
**import_database.py** ⭐ Primary setup script
- Restores all `swiss_cv_generator` collections from JSON snapshots in `data/swiss_cv_generator/`
- Skips collections that already have data (safe to run on existing setup)
- Use `--force` to drop and reimport all collections
- No OpenAI API key required

**export_database.py**
- Exports current `swiss_cv_generator` collections to `data/swiss_cv_generator/*.json`
- Run after regenerating data to update the committed snapshots
- Commit the result so new Codespaces benefit immediately

**setup_complete_database.py**
- Orchestrates full AI-powered database initialization (requires OpenAI API key)
...
**Problem:** No occupations or collections found
**Solution:**
- Restore from snapshots (fast, no API key): `python scripts/import_database.py`
- Import source occupations: `python scripts/import_cv_data.py`
- Or run full setup (needs OpenAI key): `python scripts/setup_complete_database.py`
- Verify MongoDB collections exist: `python scripts/test_db_connection.py`
